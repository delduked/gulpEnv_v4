...
gulp.task('jade', function(){
   return gulp.src('site/jade/index.jade')
      // Specifies which file is will be processed into html
      .pipe(jade())
      // Compiles the Jade file into HTML
      .pipe(gulp.dest('site'))
      // Specifies where the processed HTML file will reside
      .pipe(browserSync.reload({
         stream: true
      }))
      // Upon Jade file being compiled, the Gulp task BrowserSync will reload the page
});
// Jade Preprocessor
...
gulp.task('watch', ['jade','sass','browserSync','concat-js'], function(){
   gulp.watch('site/jade/*.jade', ['jade']);
